...
namespace NineVita
{
    public partial class MainForm : MetroForm
    {
                    ...
                    for (int i = 0; i < profile.Length; i++)
                    {
                        if (profile[i].Contains("photo") && (IsPhoto == false))
                        {
                            ...
                            catch
                            {
                                ...
                                catch { IsPhoto = false; try { File.Delete(@Application.StartupPath + @"\DATA\avatar.dat"); } catch { } }
                            }
							if (File.Exists(@Application.StartupPath + @"\DATA\avatar.dat") == false) { avatarAuth1.Image = Properties.Resources.StandardAvatar; }
                        }
                    }
                    helloUser1.Text += helloUser1.Text + username + "!";
        ...
        private void button39_Click(object sender, EventArgs e)
        {
            ...
            avatar.Visible = true;
            try { avatar.Image = avatarAuth2.Image; } catch { avatar.Image = Properties.Resources.StandardAvatar; }
        }
        ...
        private void pictureBox19_Click(object sender, EventArgs e) { UserID2.Text = Clipboard.GetText(); }
        private void pictureBox22_Click(object sender, EventArgs e) { UserID1.Text = Clipboard.GetText(); }
        ...
        private void button13_Click(object sender, EventArgs e)
        {
            ...
            panel4.Visible = false;
            avatar.Visible = true;
            try { avatar.Image = avatarAuth1.Image; } catch { avatar.Image = Properties.Resources.StandardAvatar; }
        }
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            using (RegistryKey reg = Registry.CurrentUser.CreateSubKey(@"Software\Zalexanninev15\NineVita"))
            {
                backups = Convert.ToString(reg.GetValue("BackupPath")); if (backups == "") { backups = @Application.StartupPath + @"\EWT\backups"; } bakdir.Text = backups; 
                mbnfile = Convert.ToString(reg.GetValue("Flasher")); if (mbnfile == "") { mbnfile = @Application.StartupPath + @"\flasher.mbn"; } flasherdir.Text = mbnfile;
                bakex = "img";
            ...
            if (id != "")
            {
                    ...
                    using (WebClient wc = new WebClient())
                    {
                        ...
                        for (int i = 0; i < profile.Length; i++)
                        {
                            if (profile[i].Contains("photo") && (IsPhoto == false))
                            {
								...
								if (avatar.ImageLocation != @Application.StartupPath + @"\DATA\avatar.dat") { avatar.Image = Properties.Resources.StandardAvatar; }
                            }
                            avatar.Visible = true;
                        }
                    }
                ...
                {
                    avatar.Visible = false;
                    if (File.Exists(@Application.StartupPath + @"\DATA\avatar.dat"))
                    {
                    ...
                    else { try { avatar.Image = Properties.Resources.StandardAvatar; avatar.Visible = true; } catch { } }
                }
                panel3.Visible = false;
                panel4.Visible = false;
            }
